...
public class AdmDataGen {
	...
	public static void printDate(int year, int month, int day, PrintStream out)
			throws IOException {
	...
	abstract class AbstractValueGenerator {
		...
		public void init(PrintStream out, DataGeneratorContext ctx)
				throws Exception {
			this.out = out;
			this.ctx = ctx;
		}
	...
	abstract class AbstractCollectionDataGenerator extends RandomValueGenerator {
		...
		public AbstractCollectionDataGenerator(AbstractCollectionType act,
				int min, int max) {
			this.act = act;
			this.min = min;
			this.max = max;
		...
		public void init(PrintStream out, DataGeneratorContext ctx)
			...
			if (t.getTypeTag() != ATypeTag.RECORD) {
				throw new NotImplementedException(
						"list annotation only works with record item types for now.");
			}
	...
	class ListFromArrayGenerator extends AbstractCollectionDataGenerator {
		...
		public ListFromArrayGenerator(AbstractCollectionType act,
				String[] array, int min, int max) {
			super(act, min, max);
			this.array = array;
		}
	...
	class StringFromArraySameIndexGenerator extends AbstractValueGenerator {
		...
		public StringFromArraySameIndexGenerator(String[] array,
				StringFromArrayGenerator sfag, String constructor) {
			this.array = array;
			this.sfag = sfag;
			this.constructor = constructor;
		}
	...
	class IntIntervalGenerator extends RandomValueGenerator {
		...
		public IntIntervalGenerator(int min, int max, String prefix,
				String suffix) {
			this.min = min;
			this.max = max;
			this.prefix = prefix;
	...
	class LongIntervalGenerator extends RandomValueGenerator {
		...
		public LongIntervalGenerator(long min, long max, String prefix,
				String suffix) {
			this.min = min;
			this.max = max;
			this.prefix = prefix;
	...
	class DoubleIntervalGenerator extends RandomValueGenerator {
		...
		public DoubleIntervalGenerator(double min, double max, String prefix,
				String suffix) {
			this.min = min;
			this.max = max;
			this.prefix = prefix;
	...
	abstract class DateGenerator extends RandomValueGenerator implements
			AccessibleDateGenerator {

		private int year;
		private int month;
		private int day;

		protected void generateDate(int minYear, int maxYear)
				throws IOException {
	...
	class DatetimeBetweenYearsGenerator extends DateGenerator implements
			AccessibleDatetimeGenerator {
		private final int minYear;
		private final int maxYear;
		private int hour;
	...
	class DatetimeAddRandHoursGenerator extends RandomValueGenerator {
		...
		public DatetimeAddRandHoursGenerator(int minHours, int maxHours,
				AccessibleDatetimeGenerator adtg) {
	...
	class GenFieldsIntGenerator extends RandomValueGenerator {
		...
		public GenFieldsIntGenerator(int startIndex, int minFields,
				int maxFields, String prefix) {
	...
	class RecordGenerator extends RandomValueGenerator {
		...
		public RecordGenerator(ARecordType type, RecordDataGenAnnotation annot,
				String printAfter) {
			this.recType = type;
			this.annot = annot;
		...
		public void init(PrintStream out, DataGeneratorContext ctx)
				...
				for (int i = 0; i < m; i++) {
					...
					if (rfdg == null) {
						if (ti.getTypeTag() == ATypeTag.RECORD) {
							...
							for (IRecordTypeAnnotation annot : rt
									.getAnnotations()) {
							...
							if (dga != null) {
								declaredFieldsGenerators[i] = new RecordGenerator(
										rt, dga, null);
								continue;
							}
						}
						if (declaredFieldsGenerators[i] == null) {
							throw new Exception(
									"No data generator annotation for field "
					...
					case VALFILE: {
						...
						declaredFieldsGenerators[i] = new StringFromArrayGenerator(
								a, constructor);
						break;
					}
					case LISTVALFILE: {
						ListValFileDataGen lvf = (ListValFileDataGen) rfdg;
						String[] a = ctx.getFileToLoadedDataMap().get(
								lvf.getFile());
						if (a == null) {
							a = readFileAsStringArray(lvf.getFile());
							ctx.getFileToLoadedDataMap().put(lvf.getFile(), a);
						}
						if (ti.getTypeTag() != ATypeTag.ORDEREDLIST
								&& ti.getTypeTag() != ATypeTag.UNORDEREDLIST) {
							throw new Exception(
									"list-val-file annotation cannot be used for field of type "
											+ ti.getTypeTag());
						}
						AbstractCollectionType act = (AbstractCollectionType) ti;
						declaredFieldsGenerators[i] = new ListFromArrayGenerator(
								act, a, lvf.getMin(), lvf.getMax());
						break;
					}
					case VALFILESAMEINDEX: {
						...
						for (int j = 0; j < i; j++) {
							if (recType.getFieldNames()[j].equals(vfsi
									.getSameAsField())) {
						...
						if (sfag == null) {
							throw new Exception("Couldn't find field "
									+ vfsi.getSameAsField() + " before field "
									+ recType.getFieldNames()[i]);
						}
						String constructor;
						if (ti.getTypeTag() == ATypeTag.STRING) {
						...
						declaredFieldsGenerators[i] = new StringFromArraySameIndexGenerator(
								a, sfag, constructor);
						break;
					}
					case INTERVAL: {
						...
						case INT: {
							declaredFieldsGenerators[i] = new IntIntervalGenerator(
									Integer.parseInt(fi.getMin()),
									Integer.parseInt(fi.getMax()), prefix,
									suffix);
							break;
						}
						case LONG: {
							declaredFieldsGenerators[i] = new LongIntervalGenerator(
									Long.parseLong(fi.getMin()),
									Long.parseLong(fi.getMax()), prefix, suffix);
							break;
						}
						case DOUBLE: {
							declaredFieldsGenerators[i] = new DoubleIntervalGenerator(
									Double.parseDouble(fi.getMin()),
									Double.parseDouble(fi.getMax()), prefix,
									suffix);
							break;
						}
						default: {
					...
					case INSERTRANDINT: {
						...
						declaredFieldsGenerators[i] = new InsertRandIntGenerator(
								iri.getStr1(), iri.getStr2());
						break;
					}
					case LIST: {
						...
						AbstractCollectionType act = (AbstractCollectionType) ti;
						declaredFieldsGenerators[i] = new ListDataGenerator(
								act, l.getMin(), l.getMax());
						break;
					}
					case DATEBETWEENYEARS: {
						DateBetweenYearsDataGen dby = (DateBetweenYearsDataGen) rfdg;
						declaredFieldsGenerators[i] = new DateBetweenYearsGenerator(
								dby.getMinYear(), dby.getMaxYear());
						break;
					}
					case DATETIMEBETWEENYEARS: {
						DatetimeBetweenYearsDataGen dtby = (DatetimeBetweenYearsDataGen) rfdg;
						declaredFieldsGenerators[i] = new DatetimeBetweenYearsGenerator(
								dtby.getMinYear(), dtby.getMaxYear());
						break;
					}
					case DATETIMEADDRANDHOURS: {
						...
						for (int j = 0; j < i; j++) {
							if (recType.getFieldNames()[j].equals(dtarh
									.getAddToField())) {
						...
						if (adtg == null) {
							throw new Exception("Couldn't find field "
									+ dtarh.getAddToField() + " before field "
									+ recType.getFieldNames()[i]);
						}
						declaredFieldsGenerators[i] = new DatetimeAddRandHoursGenerator(
								dtarh.getMinHour(), dtarh.getMaxHour(), adtg);
						break;
					}
					case AUTO: {
						...
						default: {
							throw new IllegalStateException(
									rfdg.getKind()
											+ " annotation is not implemented for type "
											+ ti.getTypeTag());
						}
						}
						break;
					...
					default: {
						throw new NotImplementedException(rfdg.getKind()
								+ " is not implemented");
					}
					}
				}
			...
			if (undeclaredFieldsGenerator == null) {
				UndeclaredFieldsDataGen ufdg = annot
						.getUndeclaredFieldsDataGen();
				if (ufdg != null) {
					if (!recType.isOpen()) {
						throw new Exception(
								"Cannot generate undeclared fields for closed type "
										+ recType);
					}
					undeclaredFieldsGenerator = new GenFieldsIntGenerator(
							declaredFieldsGenerators.length,
							ufdg.getMinUndeclaredFields(),
							ufdg.getMaxUndeclaredFields(),
	...
	public void init() throws IOException, ParseException, AsterixException,
			ACIDException, MetadataException, AlgebricksException {
	...
	public void dataGen() throws Exception {
		for (Map.Entry<TypeSignature, IAType> me : typeMap.entrySet()) {
			...
			if (tdg.isDataGen()) {
				...
				if (dga == null) {
					throw new Exception(
							"No data generator annotations for type " + tn);
				}
				File outFile = new File(outputDir + File.separator
						+ tdg.getOutputFileName());
