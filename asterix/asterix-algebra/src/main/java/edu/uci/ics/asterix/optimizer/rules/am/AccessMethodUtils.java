...
public class AccessMethodUtils {
    ...
    public static void appendInputVarsAndTypes(ILogicalOperator inputOp, IOptimizationContext context,
            List<LogicalVariable> vars, List<Object> types) throws AlgebricksException {
        VariableUtilities.getLiveVariables(inputOp, vars);
        IVariableTypeEnvironment typeEnv = context.getOutputTypeEnvironment(inputOp);
        for (LogicalVariable var : vars) {
            types.add(typeEnv.getVarType(var));
        }
    }
    ...
    public static UnnestMapOperator createSecondaryIndexUnnestMap(AqlCompiledDatasetDecl datasetDecl,
        ...
        if (retainInput) {
            appendInputVarsAndTypes(inputOp, context, secondaryIndexUnnestVars, secondaryIndexOutputTypes);
        }
    ...
    public static UnnestMapOperator createPrimaryIndexUnnestMap(AqlCompiledDatasetDecl datasetDecl, 
        ...
        if (retainInput) {
            appendInputVarsAndTypes(inputOp, context, primaryIndexUnnestVars, primaryIndexOutputTypes);
        }
        primaryIndexUnnestVars.addAll(primaryIndexOutputVars);
        appendPrimaryIndexTypes(datasetDecl, recordType, primaryIndexOutputTypes);
        UnnestMapOperator primaryIndexUnnestOp = new UnnestMapOperator(primaryIndexUnnestVars, new MutableObject<ILogicalExpression>(searchPrimIdxFun),
