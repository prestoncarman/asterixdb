...
public class DatasetUtils {
    ...
    public static IAType[] computePartitioningKeyType(Dataset dataset, ARecordType itemType)throws AlgebricksException {
        if (dataset.getDatasetType() == DatasetType.EXTERNAL) {
            throw new AlgebricksException("not implemented");
        }
        List<String> partitioningKeys = DatasetUtils.getPartitioningKeys(dataset);
        int numKeys = partitioningKeys.size();
        IAType[] keyTypes = new IAType[numKeys];
        for (int i = 0; i < numKeys; i++) {
        	keyTypes[i] = itemType.getFieldType(partitioningKeys.get(i));
        }
        return keyTypes;
    }
    
    public static ISerializerDeserializer[] computeTupleSerde(Dataset dataset, ARecordType itemType)throws AlgebricksException {
        if (dataset.getDatasetType() == DatasetType.EXTERNAL) {
            throw new AlgebricksException("not implemented");
        }
        List<String> partitioningKeys = DatasetUtils.getPartitioningKeys(dataset);
        int numKeys = partitioningKeys.size();
        ISerializerDeserializer[] tupleSerde = new ISerializerDeserializer[numKeys+1];
        for (int i = 0; i < numKeys; i++) {
        	IAType keyType = itemType.getFieldType(partitioningKeys.get(i));
        	tupleSerde[i] = AqlSerializerDeserializerProvider.INSTANCE.getSerializerDeserializer(keyType);
        }
        tupleSerde[numKeys] = AqlSerializerDeserializerProvider.INSTANCE.getSerializerDeserializer(itemType);
        return tupleSerde;
    }
}
