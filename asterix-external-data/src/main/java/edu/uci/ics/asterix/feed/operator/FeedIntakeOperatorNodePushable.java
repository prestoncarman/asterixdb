...
public class FeedIntakeOperatorNodePushable extends
	...
	public FeedIntakeOperatorNodePushable(FeedId feedId,
			IDatasourceReadAdapter adapter, int partition) {
	...
	public void initialize() throws HyracksDataException {
		feedInboxMonitor = new FeedInboxMonitor((IManagedFeedAdapter) adapter,
				inbox, partition);
		...
		try {
			...
			throw new HyracksDataException(
					"exception during reading from external data source", e);
		} finally {
			writer.close();
		}
	}
	...
	public void run() {
		while (true) {
			try {
				...
				case SUSPEND:
					((IManagedDataParser) adapter.getDataParser(partition))
							.getManagedTupleParser().suspend();
					break;
				case RESUME:
					((IManagedDataParser) adapter.getDataParser(partition))
							.getManagedTupleParser().resume();
					break;
				case STOP:
					((IManagedDataParser) adapter.getDataParser(partition))
							.getManagedTupleParser().stop();
					break;
				case ALTER:
					((IMutableFeedAdapter) adapter)
							.alter(((AlterFeedMessage) feedMessage)
