...
public class PullBasedTwitterAdapter extends AbstractDatasourceAdapter
		implements IDatasourceReadAdapter, IManagedFeedAdapter,
		IMutableFeedAdapter {
	...
	public void configure(Map<String, String> arguments, IAType atype)
		...
		this.atype = atype;
		String parallelProp = arguments.get(PARALLELISM);
		if (parallelProp != null) {
			parallelism = Integer.parseInt(parallelProp);
		}
	...
	public IDataParser getDataParser(int partition) throws Exception {
		if (parser == null) {
			parser = new ManagedDelimitedDataStreamParser('|', this);
			parser.initialize((ARecordType) atype, configuration, ctx);
			TweetClient tweetClient = new TweetClient(ctx.getJobletContext()
					.getApplicationContext().getNodeId(), this);
...
class TweetStream extends InputStream {
	...
	public TweetStream(TweetClient tweetClient, IHyracksTaskContext ctx)
			throws Exception {
...
class TweetClient implements IFeedClient {
	...
	private void initialize(Map<String, String> params) {
		...
		if (params.get(PullBasedTwitterAdapter.INTERVAL) != null) {
			this.timeInterval = Integer.parseInt(params
					.get(PullBasedTwitterAdapter.INTERVAL));
		}
		if (params.get(PullBasedTwitterAdapter.KEY_DELIMITER) != null) {
			this.delimiter = params.get(PullBasedTwitterAdapter.KEY_DELIMITER)
					.charAt(0);
		}
	}
	...
	public boolean next(List<String> tweets) {
		try {
			...
			if (adapter.isAlterRequested()) {
				initialize(((PullBasedTwitterAdapter) adapter)
						.getAlteredParams());
	...
	public String formFeedTuple(Object tweetObject) {
		...
		builder.append(delimiter);
		builder.append(tweet.getLocation());
		builder.append(delimiter);
