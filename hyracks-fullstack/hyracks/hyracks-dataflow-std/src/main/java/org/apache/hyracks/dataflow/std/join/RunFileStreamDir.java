...
public class RunFileStreamDir {

    private static final Logger LOGGER = Logger.getLogger(RunFileStreamDir.class.getName());

    private final String key;
    private final IFrame runFileBuffer;
    private final IFrameTupleAppender runFileAppender;
    private RunFileWriterDir runFileWriter;
     ...
     */
    public RunFileStreamDir(IHyracksTaskContext ctx, String key, IRunFileStreamStatus status) throws HyracksDataException {
        this.ctx = ctx;
        this.key = key;
        this.status = status;
    ...
    public void createRunFileWriting() throws HyracksDataException {
        ...
        runFileWriter = new RunFileWriterDir(runfile, ctx.getIOManager());
        runFileWriter.open();
