            ...
            switch (stmt.getKind()) {
                ...
                case CREATE_FUNCTION: {
                    ...
                    Function function = new Function(compiledDeclarations.getDataverseName(), cfs
                            .getFunctionIdentifier().getName(), cfs.getFunctionIdentifier().getArity(),
                            cfs.getParamList(), cfs.getFunctionBody(), cfs.getReturnType(), cfs.getDependencies(),
                            cfs.getLanguage());

                    if (AsterixBuiltinFunctions.isBuiltinCompilerFunction(cfs.getFunctionIdentifier())) {
                        throw new AsterixException("invalid name, a built-in function by this name already exists");
                    }
                    if (function.getLanguage() == Function.LANGUAGE_AQL) {
                        try {
                            FunctionUtil.getUserDefinedFunctionDecl(function);
                        } catch (Exception e) {
                    ...
                    MetadataManager.INSTANCE.addFunction(
                            ...
                            new Function(compiledDeclarations.getDataverseName(),
                                    cfs.getFunctionIdentifier().getName(), cfs.getFunctionIdentifier().getArity(), cfs
