...
public class EventUtil {
    ...
    private static Node getNodeFromId(String nodeid, Cluster cluster) {
        ...
        if (nodeid.equals(cluster.getMasterNode().getId())) {
            String ram = cluster.getMasterNode().getRam() == null ? cluster.getRam() : cluster.getMasterNode().getRam();
            ...
            return new Node(cluster.getMasterNode().getId(), cluster.getMasterNode().getIp(), ram, javaHome, logDir,
                    null, debug);
        }
