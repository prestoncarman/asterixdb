...
public class HDFSAdapter extends FileSystemBasedAdapter {
	...
	private static final Logger LOGGER = Logger.getLogger(HDFSAdapter.class
			.getName());
	...
	static {
		formatClassNames.put(INPUT_FORMAT_TEXT,
				"org.apache.hadoop.mapred.TextInputFormat");
		formatClassNames.put(INPUT_FORMAT_SEQUENCE,
			...
			try {
				for (InputSplit inputSplit : inputSplits) {
					...
					for (String datanodeLocation : dataNodeLocations) {
						...
						if (nodeControllersAtLocation == null
								|| nodeControllersAtLocation.size() == 0) {
							if (LOGGER.isLoggable(Level.INFO)) {
								LOGGER.log(
										Level.INFO,
										"No node controller found at "
												+ datanodeLocation
												+ " will look at replica location");
							}
							couldConfigureLocationConstraints = false;
						} else {
							int locationIndex = random
									.nextInt(nodeControllersAtLocation.size());
							String chosenLocation = (String) nodeControllersAtLocation
									.toArray()[locationIndex];
							locations.add(chosenLocation);
							if (LOGGER.isLoggable(Level.INFO)) {
								LOGGER.log(Level.INFO, "split : " + inputSplit
										+ " to be processed by :"
										+ chosenLocation);
							}
					...
					if (!couldConfigureLocationConstraints) {
						List<String> allNodeControllers = AsterixRuntimeUtil
								.getAllNodeControllers();
						int locationIndex = random.nextInt(allNodeControllers
								.size());
						String chosenLocation = allNodeControllers
								.get(locationIndex);
						locations.add(chosenLocation);
						...
						if (LOGGER.isLoggable(Level.INFO)) {
							LOGGER.log(
									Level.INFO,
									"No local node controller found to process split : "
											+ inputSplit
											+ " will be processed by a remote node controller:"
											+ chosenLocation);
						}
						break;
					}
				...
				if (couldConfigureLocationConstraints) {
					partitionConstraint = new AlgebricksAbsolutePartitionConstraint(
	...
	private JobConf configureJobConf() throws Exception {
		...
		conf.set("mapred.input.format.class",
				formatClassNames.get(configuration.get(KEY_INPUT_FORMAT)));
		return conf;
	}
		...
		try {
			...
			if (conf.getInputFormat() instanceof SequenceFileInputFormat) {
				...
				RecordReader reader = format
						.getRecordReader(
								(org.apache.hadoop.mapred.FileSplit) inputSplits[partition],
								conf, getReporter());
...
class HDFSStream extends InputStream {
	...
	public HDFSStream(RecordReader reader, IHyracksTaskContext ctx)
			throws Exception {
		...
		try {
			...
			throw new Exception(
					"context is not of type org.apache.hadoop.io.Text"
							+ " type not supported in sequence file format",
					cce);
		}
		initialize();
	}
