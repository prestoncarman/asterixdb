...
public final class AqlRewriter {
    ...
    private void inlineDeclaredUdfs() throws AsterixException {
        ...
        List<FunctionIdentifier> funIds = new ArrayList<FunctionIdentifier>();
    ...
    private void buildOtherUdfs(Expression expression, List<FunctionDecl> functionDecls,
            List<FunctionIdentifier> declaredFunctions) throws AsterixException {
        if (expression == null) {
            return;
        }

        List<FunctionIdentifier> functionCalls = getFunctionCalls(expression);
        for (FunctionIdentifier funId : functionCalls) {
            if (AsterixBuiltinFunctions.isBuiltinCompilerFunction(funId)) {
                continue;
            }

            if (AsterixBuiltinFunctions.isBuiltinCompilerFunction(new FunctionIdentifier(
                    AlgebricksBuiltinFunctions.ALGEBRICKS_NS, funId.getName(), false))) {
                continue;
            }
            ...
            FunctionDecl functionDecl = getUserDefinedFunctionDecl(funId);
    ...
    private FunctionDecl getUserDefinedFunctionDecl(FunctionIdentifier funId) throws AsterixException {
        Function function = MetadataManager.INSTANCE.getFunction(mdTxnCtx, dataverseName, funId.getName(),
                funId.getArity());
        if (function == null) {
            throw new AsterixException(" unknown function " + funId);
        }
        return FunctionUtil.getUserDefinedFunctionDecl(function);
    }

    private List<FunctionIdentifier> getFunctionCalls(Expression expression) throws AsterixException {
        Map<FunctionIdentifier, DfsColor> color = new HashMap<FunctionIdentifier, DfsColor>();
        Map<FunctionIdentifier, List<FunctionIdentifier>> arcs = new HashMap<FunctionIdentifier, List<FunctionIdentifier>>();
        GatherFunctionCalls gfc = new GatherFunctionCalls();
        expression.accept(gfc, null);
        List<FunctionIdentifier> calls = gfc.getCalls();
        return calls;
    }
    ...
    private void checkRecursivity(List<FunctionDecl> fdecls) throws AsterixException {
        ...
        for (FunctionIdentifier a : arcs.keySet()) {
    ...
    private void checkRecursivityDfs(FunctionIdentifier a, Map<FunctionIdentifier, List<FunctionIdentifier>> arcs,
            Map<FunctionIdentifier, DfsColor> color) throws AsterixException {
        color.put(a, DfsColor.GRAY);
        List<FunctionIdentifier> next = arcs.get(a);
        if (next != null) {
            for (FunctionIdentifier f : next) {
    ...
    private static class GatherFunctionCalls implements IAqlExpressionVisitor<Void, Void> {
        ...
        private final List<FunctionIdentifier> calls = new ArrayList<FunctionIdentifier>();
        ...
        public List<FunctionIdentifier> getCalls() {
            return calls;
        }
