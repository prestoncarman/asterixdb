...
import edu.uci.ics.asterix.api.http.servlet.KVServlet;

public class NCBootstrapImpl implements INCBootstrap {
	private static final Logger LOGGER = Logger.getLogger(NCBootstrapImpl.class
			.getName());
	...
	public void start() throws Exception {
		...
		AsterixNodeState ns = (AsterixNodeState) proxy
				.getAsterixNodeState(nodeId);
		apiNodeDataServer = new ThreadedServer(ns.getAPINodeDataServerPort(),
				new NodeDataClientThreadFactory());
		apiNodeDataServer.start();
		// ------ Added for KVS Servlet ------
		server = new Server(ns.getKvServicePort());
		ServletContextHandler context = new ServletContextHandler(
				ServletContextHandler.SESSIONS);
		context.setContextPath("/");
		server.setHandler(context);
		context.addServlet(new ServletHolder(new KVServlet()), "/kvs/*");
		server.start();
		// -----------------------------
	}
