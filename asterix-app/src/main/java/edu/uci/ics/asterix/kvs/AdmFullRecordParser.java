...
public class AdmFullRecordParser  {
   ...
   //private ByteBuffer frame = ctx.allocateFrame();
    ...
    public AdmFullRecordParser(ARecordType rt){
    	this.recType = rt;
    }
    
    public void parseOneRecord(InputStream in, ArrayTupleBuilder tb,  DataOutput dos) throws HyracksDataException {
        ...
        try {
            //while (true) {
                ...
                if (!parseAdmInstance(recType, true, dos)) {
                    System.out.println(">>>>> !!!! Could not parse Adm Insrance");
                	return;
                }
