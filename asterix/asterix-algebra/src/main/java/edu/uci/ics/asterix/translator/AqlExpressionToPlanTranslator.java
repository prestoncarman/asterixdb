...
public class AqlExpressionToPlanTranslator extends AbstractAqlTranslator
	...
	private static String dataverseName;
	...
	public ILogicalPlanAndMetadata translate(Query expr,
		...
		if (compiledDeclarations == null) {
			compiledDeclarations = compileMetadata(mdTxnCtx, expr
					.getPrologDeclList(), true);
		}
		if (!compiledDeclarations.isConnectedToDataverse())
			compiledDeclarations.connectToDataverse(compiledDeclarations
					.getDataverseName());

		dataverseName = compiledDeclarations.getDataverseName();
		...
		if (outputDatasetName == null) {
			...
			AqlCompiledDatasetDecl adecl = compiledDeclarations
			...
			AqlDataSource dataSource = new AqlDataSource(sourceId, adecl,
					itemType);
	...
	public Pair<ILogicalOperator, LogicalVariable> visitCallExpr(
		...
		IFunctionInfo finfo;
		AbstractFunctionCallExpression f;
		if (FunctionUtils.isAggregateFunction(dataverseName, fid)) {
			f = AsterixBuiltinFunctions.makeAggregateFunctionExpression(fi,
					args);
		} else if (FunctionUtils.isUnnestingFunction(dataverseName,fid)) {
					...
					if (i == 0) {
						...
						if (op.isBroadcastOperand(i)) {
							...
							c
									.getAnnotations()
									.put(
											BroadcastExpressionAnnotation.BROADCAST_ANNOTATION_KEY,
											bcast);
						}
					} else {
	...
	private AbstractFunctionCallExpression createFunctionCallExpressionForBuiltinOperator(
		...
		return new ScalarFunctionCallExpression(FunctionUtils
				.getFunctionInfo(fid));
	}
	...
	private Pair<ILogicalOperator, LogicalVariable> produceFlwrResult(
		...
		if (isTop) {
			...
			return new Pair<ILogicalOperator, LogicalVariable>(resOpRef
					.getValue(), resVar);
		} else {
	...
	private Pair<ILogicalOperator, LogicalVariable> aggListify(
			...
			boolean bProject) {
		AggregateFunctionCallExpression funAgg = FunctionUtils
				.makeAggregateFunctionExpression(
