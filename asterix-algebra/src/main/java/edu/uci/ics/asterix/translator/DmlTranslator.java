...
public class DmlTranslator extends AbstractAqlTranslator {
	...
	public DmlTranslator(MetadataTransactionContext mdTxnCtx,
			List<Statement> aqlStatements) {
		this.mdTxnCtx = mdTxnCtx;
		this.aqlStatements = aqlStatements;
	}

	public void translate() throws AlgebricksException, RemoteException,
	...
	private List<ICompiledDmlStatement> compileDmlStatements()
		...
		for (Statement stmt : aqlStatements) {
			...
			case LOAD_FROM_FILE: {
				...
				String adapterName = loadStmt.getAdapter();
				Adapter adapter = compiledDeclarations.getAdapter(adapterName);

				CompiledLoadFromFileStatement cls = new CompiledLoadFromFileStatement(
						loadStmt.getDatasetName().getValue(), adapter,
						loadStmt.getProperties(),
						loadStmt.dataIsAlreadySorted());
				dmlStatements.add(cls);
				// Also load the dataset's secondary indexes.
				List<Index> datasetIndexes = MetadataManager.INSTANCE
						.getDatasetIndexes(mdTxnCtx,
				...
				for (Index index : datasetIndexes) {
					...
					// 'index'.
					CompiledCreateIndexStatement cis = new CompiledCreateIndexStatement(
							index.getIndexName(), index.getDatasetName(),
							index.getKeyFieldNames(), index.getIndexType());
			...
			case WRITE_FROM_QUERY_RESULT: {
				...
				CompiledWriteFromQueryResultStatement clfrqs = new CompiledWriteFromQueryResultStatement(
						st1.getDatasetName().getValue(), st1.getQuery(),
						st1.getVarCounter());
			...
			case CREATE_INDEX: {
				...
				// Assumptions: We first processed the DDL, which added the
				// secondary index to the metadata.
				// If the index's dataset is being loaded in this 'session',
				// then let the load add
				// the CompiledCreateIndexStatement to dmlStatements, and don't
				// add it again here.
				// It's better to have the load handle this because:
				// 1. There may be more secondary indexes to load, which were
				// possibly created in an earlier session.
				// 2. If the create index stmt came before the load stmt, then
				// we would first create an empty index only to load it again
				// later.
				// This may cause problems because the index would be considered
				// loaded (even though it was loaded empty).
				...
				if (cis.getNeedToCreate()) {
					CompiledCreateIndexStatement ccis = new CompiledCreateIndexStatement(
							cis.getIndexName().getValue(), cis.getDatasetName()
									.getValue(), cis.getFieldExprs(),
							cis.getIndexType());
			...
			case INSERT: {
				...
				CompiledInsertStatement clfrqs = new CompiledInsertStatement(is
						.getDatasetName().getValue(), is.getQuery(),
						is.getVarCounter());
				dmlStatements.add(clfrqs);
				break;
			}
			case DELETE: {
				...
				CompiledDeleteStatement clfrqs = new CompiledDeleteStatement(
						ds.getVariableExpr(), ds.getDatasetName(),
						ds.getCondition(), ds.getDieClause(),
						ds.getVarCounter(), compiledDeclarations);
				dmlStatements.add(clfrqs);
				break;
			}
			...
			case BEGIN_FEED: {
				...
				CompiledBeginFeedStatement cbfs = new CompiledBeginFeedStatement(
						bfs.getDatasetName(), bfs.getQuery(),
						bfs.getVarCounter());
				dmlStatements.add(cbfs);
				Dataset dataset;
				try {
					dataset = MetadataManager.INSTANCE.getDataset(mdTxnCtx,
							compiledDeclarations.getDataverseName(), bfs
			...
			case CONTROL_FEED: {
				...
				CompiledControlFeedStatement clcfs = new CompiledControlFeedStatement(
						cfs.getOperationType(), cfs.getDatasetName(),
	...
	public static class CompiledCreateIndexStatement implements
		...
		public CompiledCreateIndexStatement(String indexName,
				String datasetName, List<String> keyFields, IndexType indexType) {
	...
	public static class CompiledLoadFromFileStatement implements
		...
		public CompiledLoadFromFileStatement(String datasetName,
				Adapter adapter, Map<String, String> properties,
	...
	public static class CompiledInsertStatement implements
		...
		public CompiledInsertStatement(String datasetName, Query query,
				int varCounter) {
			this.datasetName = datasetName;
			this.query = query;
			this.varCounter = varCounter;
	...
	public static class CompiledBeginFeedStatement implements
		...
		public CompiledBeginFeedStatement(Identifier datasetName, Query query,
				int varCounter) {
			this.datasetName = datasetName;
			this.query = query;
			this.varCounter = varCounter;
	...
	public static class CompiledControlFeedStatement implements
		...
		public CompiledControlFeedStatement(OperationType operationType,
				Identifier datasetName, Map<String, String> alteredParams) {
			this.datasetName = datasetName;
	...
	public static class CompiledDeleteStatement implements
		...
		public CompiledDeleteStatement(VariableExpr var, Identifier dataset,
		...
		public Query getQuery() throws AlgebricksException {
			...
			LiteralExpr argumentLiteral = new LiteralExpr(new StringLiteral(
					datasetName));
			arguments.add(argumentLiteral);

			CallExpr callExpression = new CallExpr(new AsterixFunction(
					"dataset", 1), arguments);
			...
			AqlCompiledDatasetDecl aqlDataset = compiledDeclarations
