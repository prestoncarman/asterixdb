...
public abstract class AbstractAqlTranslator {
	...
	protected static final Map<String, BuiltinType> builtinTypeMap = AsterixBuiltinTypeMap
			.getBuiltinTypes();;

	
	public void validateOperation(Dataverse dataverse, Statement stmt)
			throws AsterixException {
		String dataverseName = dataverse != null ? dataverse.getDataverseName()
				: null;
		if (dataverseName != null
			...
			case DELETE:
				...
				message = " Operation  " + stmt.getKind()
						+ " not permitted in system dataverse-"
						+ MetadataConstants.METADATA_DATAVERSE_NAME;
				break;
			case FUNCTION_DROP:
				FunctionSignature signature = ((FunctionDropStatement) stmt)
				...
				if (dataverseName
						.equals(MetadataConstants.METADATA_DATAVERSE_NAME)
						&& AsterixBuiltinArtifactMap.isSystemProtectedArtifact(
								ARTIFACT_KIND.FUNCTION, fId)) {
					invalidOperation = true;
					message = "Invalid Operation cannot drop function "
							+ signature + " (protected by system)";
				}
				break;
			case NODEGROUP_DROP:
				NodeGroupDropStatement nodeGroupDropStmt = (NodeGroupDropStatement) stmt;
				String nodegroupName = nodeGroupDropStmt.getNodeGroupName()
						.getValue();
				if (AsterixBuiltinArtifactMap.isSystemProtectedArtifact(
						ARTIFACT_KIND.NODEGROUP, nodegroupName)) {
					message = "Invalid Operation cannot drop nodegroup "
			...
			case DATAVERSE_DROP:
				...
				if (dvName.equals(MetadataConstants.METADATA_DATAVERSE_NAME)) {
					message = "Invalid Operation cannot drop dataverse "
							+ dvName + " (protected by system)";
					invalidOperation = true;
				}
				break;
			case DATASET_DROP:
				...
				if (AsterixBuiltinArtifactMap.isSystemProtectedArtifact(
					...
					invalidOperation = true;
					message = "Invalid Operation cannot drop dataset "
